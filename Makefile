...
#*************************************************************************
M3_CROSS_COMPILE    :=$(CROSS_COMPILE)
...
USER_AS_DEFINES     :=
USER_AS_FLAGS       :=-mcpu=cortex-m3 -mthumb -D__ASSEMBLY__ -x assembler-with-cpp -MD -c -D__CMSIS_RTOS -D__M3_OS__ -D__M3BOOT__ -fno-stack-protector
USER_AS_OPTIM_FLAGS :=
USER_C_FLAGS        :=-mcpu=cortex-m3 -mthumb -MD -c -D__CMSIS_RTOS -D__M3BOOT__ -fno-stack-protector
USER_LD_FLAGS       :=-X -EL -N 
LINKER_SCRIPT_SRC   :=$(BSP_DIR)/boot.ld.S
LINKER_SCRIPT       :=$(BSP_DIR)/boot.ld

OBB_PRODUCT_DELIVERY_DIR := $(BSP_DIR)/output
LOCAL_OBJ_PATH := $(BSP_DIR)/output
...
OBC_LOCAL_INC_DIR  := \
	$(BSP_MCORE_DIR)/include \
	$(BSP_MCORE_DIR)/config \
	$(BSP_MCORE_DIR)/boot \
...
ifneq ($(strip $(OBJ_AS_DIRS)),)
	...
	$(Q)mkdir -p $(OBB_PRODUCT_DELIVERY_DIR)/obj
	$(Q)mkdir -p $(OBB_PRODUCT_DELIVERY_DIR)/lib
